...
@interface TradPlusMaioSDKLoader()<MaioDelegate>
{
...
@implementation TradPlusMaioSDKLoader

+ (TradPlusMaioSDKLoader *)sharedInstance
{
    static TradPlusMaioSDKLoader *loader = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        loader = [[TradPlusMaioSDKLoader alloc] init];
    });
    return loader;
}
...
- (void)showAdapterInfo
{
    ...
    [adapterInfo appendFormat:@"Ad Network[三方源]:%@，",@"Maio"];
    [adapterInfo appendFormat:@"Adapter version[Adapter版本]:%@，",TP_MaioAdapter_Version];
    [adapterInfo appendFormat:@"Compatible version[适配sdk版本]:%@，",TP_MaioAdapter_PlatformSDK_Version];
    if(version != nil)
    {
             ...
             delegate:(id <TPSDKLoaderDelegate>)delegate
{
    ...
    [Maio startWithMediaId:appID delegate:nil];
    self.isIniting = NO;
    self.didInit = YES;
    [self initFinish];
    ...
    dic[@"as"] = [NSString stringWithFormat:@"%ld",(long)NETWORK_MAIO];
    dic[@"asn"] = [MsCommon channelID2Name:NETWORK_MAIO];
    dic[@"ec"] = @"1";
